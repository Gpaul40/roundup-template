...
import { groupConfig, isAdmin } from '@/config/group'
...
export async function payFineAction(fineId: string) {
  const session = await requireSession()
  if (!isAdmin(session.username)) throw new Error('Admin only')
...
export async function uploadAppImageAction(key: string, formData: FormData): Promise<{ url: string } | { error: string }> {
  const session = await requireSession()
  if (!isAdmin(session.username)) return { error: 'Admin only' }
...
export async function detonateOrganiserAction(data: {
  ...
  const session = await requireSession()
  if (!isAdmin(session.username) && session.username !== data.currentOrganiser) {
  ...
  // Next organiser gets their normal cycle PLUS whatever days were remaining
  const newEnd = new Date()
  newEnd.setDate(newEnd.getDate() + groupConfig.cycleDays + Math.max(0, data.daysRemaining))
