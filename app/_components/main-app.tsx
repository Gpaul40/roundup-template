...
export function MainApp({ currentUser }: MainAppProps) {
  ...
  const memberRankMap = membersState.reduce((acc, m) => {
    ...
    acc[m.name] = getMemberRank(evCount, fCount, m.name)
    return acc
  }, {} as Record<string, ReturnType<typeof getMemberRank>>)
  ...
  return (
    <div className="min-h-screen bg-background pb-8">
        ...
        <div className="px-4 py-3">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-base font-bold text-gold-gradient">{groupConfig.appName}</h1>
      ...
      <main className="px-4 py-4 space-y-4 max-w-lg mx-auto">
        {/* Admin Override - only visible to admins */}
        {isAdmin && (
        ...
        <div className="relative glass-card rounded-xl overflow-hidden animate-breathe" style={{boxShadow: '0 0 20px 4px rgba(212,175,55,0.25)'}}>
          ...
          {groupConfig.groupPhoto ? (
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-amber-950/70 via-black/90 to-black" />
          )}
          {/* Content */}
              ...
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground truncate">
                    Kills {currentOrganiser}&apos;s turn · {groupConfig.currency}{groupConfig.fineAmount} fine · passes to {nextOrganiser}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="glass-card rounded-2xl p-6 w-full max-w-sm border border-red-500/50 shadow-2xl space-y-4 text-center">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-2 leading-relaxed">
                  This will issue a <span className="text-red-400 font-bold">{groupConfig.currency}{groupConfig.fineAmount} fine</span> to {currentOrganiser} and hand the role to{' '}
                  <span className="text-primary font-bold">{nextOrganiser}</span>, who gets{' '}
                  <span className="text-secondary font-bold">{groupConfig.cycleDays} + {timeLeft.days} days</span> to organise.
                </p>
              </div>
              ...
              <div className="glass-card rounded-2xl p-6 w-full max-w-sm border border-primary/50 shadow-2xl space-y-5 text-center">
                ...
                <div className="p-3 rounded-xl bg-primary/10 border border-primary/30">
                  ...
                  <p className="font-bold text-primary">
                    {eventDateTime.toLocaleDateString(groupConfig.dateLocale, { weekday: 'long', day: 'numeric', month: 'long' })}
                  </p>
        ...
        {showVoting && currentProposal ? (
                    ...
                    return (
                      <tr key={opt.id} className={`border-b border-border/50 ${isMajority ? 'bg-emerald-500/10' : i % 2 === 0 ? 'bg-muted/10' : ''}`}>
                        <td className="p-2 whitespace-nowrap">
                          <span className="font-medium">{new Date(opt.date + 'T00:00:00').toLocaleDateString(groupConfig.dateLocale, { weekday: 'short', day: 'numeric', month: 'short' })}</span>
            ...
            {majorityDateId && (() => {
              ...
              return (
                <div className="p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/30 text-sm">
                  ...
                  {new Date(best.date + 'T00:00:00').toLocaleDateString(groupConfig.dateLocale, { weekday: 'long', day: 'numeric', month: 'short' })} at {best.time}
          ...
          <div className="glass-card rounded-xl p-5">
            ...
            {!showEventForm && currentUser !== currentOrganiser && !isAdmin && (
              <p className="text-xs text-muted-foreground">Only {currentOrganiser} can submit an event this cycle.</p>
            )}
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  <p className="text-xs text-muted-foreground uppercase tracking-wider">
                    Proposed Dates — {cycleInfo.cycleStartDate.toLocaleDateString(groupConfig.dateLocale, { day: 'numeric', month: 'short' })} to {cycleInfo.cycleEndDate.toLocaleDateString(groupConfig.dateLocale, { day: 'numeric', month: 'short', year: 'numeric' })}
                  </p>
                      ...
                      {[...calendarDates].sort((a, b) => a.getTime() - b.getTime()).map((d) => {
                        ...
                        return (
                          <div key={key} className="flex items-center gap-2">
                            <span className="text-sm flex-1">{d.toLocaleDateString(groupConfig.dateLocale, { weekday: 'short', day: 'numeric', month: 'short' })}</span>
                            <Input
                              type="time"
                              value={dateTimes[key] || ''}
        ...
        <div className="glass-card rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-lg font-bold text-amber-400">{groupConfig.currency}{totalOutstanding}</span>
          </div>
              ...
              {outstandingFines.slice(0, 3).map((fine) => (
                <div key={fine.id} className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
                  ...
                  {isAdmin ? (
                    ...
                    >
                      {groupConfig.currency}{fine.amount}
                    </Button>
                  ) : (
        ...
        <div className="glass-card rounded-xl overflow-hidden">
          ...
          <div className="relative h-52 w-full overflow-hidden bg-muted/20">
            {rouletteBanner
              ? <img src={rouletteBanner} alt="Fine fund banner" className="w-full h-full object-cover object-center" />
              : groupConfig.fineFund.image
                ? <img src={groupConfig.fineFund.image} alt="Fine fund banner" className="w-full h-full object-cover object-center" />
                : <div className="w-full h-full bg-gradient-to-br from-red-950/70 via-black/90 to-black" />
            }
            ...
            <div className="absolute bottom-3 left-4 flex items-center gap-2">
              <span className="text-2xl">{groupConfig.fineFund.emoji}</span>
              <span className="text-base font-bold text-white tracking-wide uppercase">{groupConfig.fineFund.title}</span>
            </div>
          </div>
          {/* Rule breakdown */}
          <div className="p-4 space-y-3">
            <div className="flex items-start gap-3">
              <span className="text-amber-400 font-bold text-lg leading-none mt-0.5">{groupConfig.currency}{groupConfig.fineAmount}</span>
            ...
            {groupConfig.fineFund.rules.map((rule, i) => (
              <div key={i}>
            ...
            {eventsState.slice(0, 5).map((event) => (
              <div key={event.id} className="p-3 bg-muted/30 rounded-lg">
                ...
                <p className="text-xs text-muted-foreground mb-1">
                  by {event.organiserName} - {new Date(event.date).toLocaleDateString(groupConfig.dateLocale, { day: 'numeric', month: 'short' })}
                </p>
